...
def stage2_page():
    ...
    # 3. Load Data
    data_path = "./data/stage2_network.csv"
...
""", language="python")
    ...
    if st.button("⚡ Kompilasi & Jalankan Visualisasi Jaringan", key="stage2_run_code"):
        ...
        if clean_A == "source_country" and clean_B == "destination_country":
            ...
            # Node sizing & coloring based on degree centrality
            ...
            # Color gradient: hub node gets bright gold/amber, others get cyan/blue/slate
            node_colors = []
            for n in node_list:
                if degrees[n] == max_deg or "UNITED STATES" in str(n).upper() or "USA" in str(n).upper():
                    node_colors.append("#fbbf24")  # Glowing Amber for Central Hub
                elif degrees[n] > max_deg * 0.4:
                    node_colors.append("#38bdf8")  # Cyan for High Degree
                else:
                    node_colors.append("#334155")  # Dark Slate for Minor Nodes
            ...
            # Find and display top connected hub
...
Berdasarkan peta topologi jaringan di atas, ke negara/entitas manakah seluruh pusaran transaksi ini berkonvergensi?
    ...
    if st.button("🚀 Verifikasi Temuan Hub", key="stage2_verify_btn"):
        cleaned_ans = answer.strip().upper()
        valid_targets = ["UNITED STATES", "US", "USA", "UNITED STATES OF AMERICA", "AMERIKA", "AMERIKA SERIKAT"]

        if any(target in cleaned_ans for target in valid_targets):
